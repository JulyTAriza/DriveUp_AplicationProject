...
public User save(User user) {
        // 1️⃣ Validamos si el email ya existe antes de guardar
        if (userRepository.existsByEmail(user.getEmail())) {
            throw new IllegalArgumentException("Ya existe un usuario con este correo electrónico: " + user.getEmail());
        }

        // 2️⃣ Guardamos el usuario en la BD
