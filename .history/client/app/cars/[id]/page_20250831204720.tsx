...
export default function CarDetailsPage({ params }: CarDetailsPageProps) {
  ...
  // 👉 Si no hay imágenes cargadas, la principal queda vacía, y las miniaturas muestran defaults
  const mainImages = car.imagePaths?.length ? car.imagePaths : [];
    ...
    <div className="container mx-auto p-6">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-10 bg-white rounded-2xl shadow-lg p-6">
        ...
        <div className="flex flex-col gap-4">
          {/* Íconos arriba */}
          <div className="flex justify-end gap-4 text-gray-600 mb-2">
            <a
          ...
          {/* Imagen principal */}
          <div className="relative w-full h-96 rounded-xl overflow-hidden shadow-lg group bg-gray-100 flex items-center justify-center">
            {mainImages.length > 0 ? (
              <Image
          ...
          <div className="grid grid-cols-3 sm:grid-cols-4 gap-3">
            {(mainImages.length > 0 ? mainImages : defaultThumbnails).map(
              (img, index) => (
                <div
                  key={index}
                  onClick={() => mainImages.length > 0 && setCurrentIndex(index)}
                  className={`relative w-full h-24 rounded-lg overflow-hidden shadow-md cursor-pointer border-2 transition ${
                    index === currentIndex && mainImages.length > 0
                      ? "border-green-500"
