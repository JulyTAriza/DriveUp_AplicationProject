...
export default function CreateReservationModal({
  ...
  // 🔹 Asegura formato YYYY-MM-DD aunque el usuario escriba distinto
  const normalizeDate = (value: string) => {
    if (!value) return "";
    const d = new Date(value);
    if (isNaN(d.getTime())) return value; // si no es válido, lo dejamos igual
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      const newRes: NewReservation = {
        ...
        pickUp,
        rentalStart: normalizeDate(rentalStart),
        rentalEnd: normalizeDate(rentalEnd),
        status: "PENDIENTE",
      };
