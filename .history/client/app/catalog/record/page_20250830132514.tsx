...
export default function HistoryPage() {
  ...
  const handleRating = async (reservationId: number, stars: number) => {
    try {
      const updated = await reservationApi.updateStars(reservationId, stars);
      setReservations((prev) =>
        prev.map((item) =>
          item.reservation.id === reservationId
            ? { ...item, reservation: { ...item.reservation, stars } }
            : item
        )
      );
    } catch (err) {
  ...
  // ❤️ Favoritos
  const toggleFavorite = async (reservationId: number, current: boolean) => {
    try {
      const updated = await reservationApi.updateFavorite(reservationId, !current);
      setReservations((prev) =>
        prev.map((item) =>
          item.reservation.id === reservationId
            ? { ...item, reservation: { ...item.reservation, favorite: !current } }
            : item
        )
      );
    } catch (err) {
              ...
              >
                ...
                <div className="flex items-center gap-1 mb-2">
                  {[1, 2, 3, 4, 5].map((star) => (
                    <button
                      key={star}
                      onClick={() => handleRating(reservation.id, star)}
                ...
                <button
                  onClick={() =>
                    toggleFavorite(reservation.id, reservation.favorite || false)
                  }
