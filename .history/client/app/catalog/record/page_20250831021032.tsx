...
export default function HistoryPage() {
  ...
  useEffect(() => {
    const fetchReservations = async () => {
      try {
        ...
        withCars.sort(
          (a, b) =>
            new Date(b.reservation.rentalStart).getTime() -
            new Date(a.reservation.rentalStart).getTime()
        );
  ...
  const filteredReservations = reservations.filter(({ reservation }) => {
    const startDate = new Date(reservation.rentalStart);
    const endDate = new Date(reservation.rentalEnd);

    const afterStart = startFilter ? startDate >= new Date(startFilter) : true;
    const beforeEnd = endFilter ? endDate <= new Date(endFilter) : true;
    ...
    <div className="flex min-h-screen bg-gray-100">
      ...
      <main className="flex-1 p-6">
        ...
        {filteredReservations.length === 0 ? (
          <p className="text-gray-600">No tienes reservas en este rango de fechas.</p>
        ) : (
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {filteredReservations.map(({ reservation, car }) => (
              <div
                key={reservation.id}
