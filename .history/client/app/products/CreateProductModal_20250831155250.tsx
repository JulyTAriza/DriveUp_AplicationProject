...
export default function CreateCarModal({
  ...
  // useEffect que se encarga de cargar las categorías y de inicializar el formulario.
  useEffect(() => {
    // Si el modal se cierra, revoca las URLs de las vistas previas para liberar memoria.
    if (!isOpen) {
    ...
    // Inicializa el formulario cuando el modal se abre.
    if (car) {
      setFormData({
        name: car.name,
  ...
  // useEffect adicional para establecer la categoría una vez que las categorías han cargado.
  useEffect(() => {
    if (car && categories.length > 0) {
      const carCategoryExists = categories.some(cat => cat.id === car.category?.id);
      
      setFormData(prev => ({
        ...prev,
        category_id: carCategoryExists ? car.category.id : categories[0].id,
      }));
