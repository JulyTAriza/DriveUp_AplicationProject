...
const handleDelete = async (id: number) => {
  if (confirm("¿Seguro que quieres eliminar este auto?")) {
    try {
      // ✅ Si la API es exitosa, esto funcionará
      await carApi.delete(id);
      
      // ✅ Actualiza el estado para eliminar el auto de la UI
      setCars((prev) => prev.filter((c) => c.id !== id));
      
      alert("¡Auto eliminado con éxito!");
    } catch (err) {
      // ✅ Si la API falla, esta alerta se mostrará
      console.error("Error eliminando auto:", err);
      alert("Hubo un error al eliminar el auto. Por favor, revisa la consola para más detalles.");
    }
  }
};
