...
export default function CreateCarModal({
  ...
  // useEffect para cargar las categorías una vez que el modal se abre.
  useEffect(() => {
    if (!isOpen) {
      // Limpia las URLs de las nuevas imágenes al cerrar el modal.
  ...
  }, [isOpen, categories.length, loadingCategories, newImagePreviews]);

  // useEffect para inicializar el formulario con los datos del auto cuando las categorías ya están cargadas.
  useEffect(() => {
    if (car && categories.length > 0) {
      ...
      setFormData({
        ...
        category_id: car.category?.id || categories[0].id,
        images: [],
      ...
      setNewImagePreviews([]);
    } else if (!car && categories.length > 0) {
      // Modo creación: reinicia el formulario y establece una categoría por defecto.
      resetForm();
