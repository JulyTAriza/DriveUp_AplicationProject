...
export default function CreateCarModal({
  ...
  useEffect(() => {
    // Si el modal se cierra, reinicia el estado y limpia la URL de la imagen
    if (!isOpen) {
      resetForm();
      ...
      try {
        ...
        if (car) {
          ...
          setFormData({
            ...
            images: [], // Las imágenes existentes no se guardan en el estado del formulario
            characteristics: car.characteristics || [],
          });
          setCharacteristicsInput((car.characteristics || []).join(", "));
          // Muestra la primera imagen existente como vista previa
    ...
    // Limpia la URL del objeto al cerrar el modal
    return () => {
      if (previewImage) {
  ...
  const handleFileChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    // Al seleccionar nuevos archivos, crea un URL para la vista previa del primero
    if (files.length > 0) {
              ...
              {previewImage && (
                <div className="mt-4">
                  ...
                  <img
                    ...
                    className="w-32 h-auto rounded-md shadow-md object-cover"
                  />
                </div>
              )}
            </div>
