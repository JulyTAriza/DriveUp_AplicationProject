...
export default function CreateCarModal({
  ...
  // Estado para las imágenes que ya están en el servidor (URLs)
  const [existingImages, setExistingImages] = useState<string[]>([]);
  // Estado para las nuevas imágenes subidas (URLs blob)
  const [newImagePreviews, setNewImagePreviews] = useState<string[]>([]);

  // useEffect 1: Se encarga de la inicialización del formulario cuando el prop 'car' cambia.
  useEffect(() => {
    if (car) {
      ...
      setFormData({
        ...
        category_id: car.category?.id || 0, // Inicia con 0 si la categoría no existe.
        images: [],
        characteristics: car.characteristics || [],
      });
      setCharacteristicsInput((car.characteristics || []).join(", "));
      // Muestra las imágenes existentes como vista previa inicial.
      setExistingImages(car.images || []);
      setNewImagePreviews([]);
    } else {
      // Modo creación: reinicia el formulario.
      resetForm();
    }
  }, [car]); // Se ejecuta solo cuando el objeto 'car' cambia.

  // useEffect 2: Se encarga de la lógica del modal y la carga de categorías.
  useEffect(() => {
    // Si el modal se cierra, limpia las URLs de las nuevas imágenes.
    if (!isOpen) {
    ...
    const fetchCategories = async () => {
      try {
        ...
        // Si el formulario no tiene categoría, la establece.
        if (formData.category_id === 0 && fetchedCategories.length > 0) {
            setFormData((prev) => ({
                ...prev,
    ...
    // Solo carga las categorías si no se han cargado aún.
  ...
  const handleFileChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    // Genera URLs para la vista previa de cada archivo
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    // Si hay nuevas imágenes seleccionadas, las añade al FormData
    if (formData.images && formData.images.length > 0) {
      ...
      // Si no hay nuevas imágenes, envía solo los datos del auto para actualizar
      data.append("car", JSON.stringify(carData));
    }
