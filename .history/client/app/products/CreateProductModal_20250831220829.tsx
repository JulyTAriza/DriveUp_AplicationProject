...
export default function CreateCarModal({
  ...
  useEffect(() => {
    ...
    setCategoriesError(null);
    setSubmitError(null); // ✅ Limpia el error de envío al abrir
    fetchCategories();
  ...
  const resetForm = () => {
    ...
    newImagePreviews.forEach((url) => URL.revokeObjectURL(url));
    setNewImagePreviews([]);
    setSubmitError(null); // ✅ Limpia el error al resetear
  };
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    setSubmitError(null); // ✅ Limpia el error al enviar
    ...
    if (!formData.category_id || formData.category_id === 0) {
      setSubmitError("Selecciona una categoría válida.");
      return;
    }
    ...
    try {
      ...
      if (err.response && err.response.data) {
        ...
        setSubmitError("Hubo un error guardando el auto. Por favor, asegurate de no crear autos existentes.");
      }
    }
  };
