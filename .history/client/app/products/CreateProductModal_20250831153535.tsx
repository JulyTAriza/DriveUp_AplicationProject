...
export default function CreateCarModal({
  ...
  // Nuevo estado para la vista previa de las imágenes
  const [previewImages, setPreviewImages] = useState<string[]>([]);
  // Estado para las imágenes existentes en modo de edición
  const [existingImages, setExistingImages] = useState<string[]>([]);

  // ✅ ÚNICO useEffect para manejar todo: carga de categorías e inicialización del formulario
  useEffect(() => {
    // Si el modal se cierra, reinicia el estado y limpia las URLs de las imágenes
    if (!isOpen) {
      resetForm();
    ...
    const fetchCategories = async () => {
      try {
        ...
        if (car) {
          ...
          // Muestra las imágenes existentes como vista previa
    ...
    // Limpia las URLs de los objetos al cerrar el modal
    return () => {
                  ...
                  <div className="flex flex-wrap gap-2">
                    {previewImages.map((src, index) => (
                      <img
                        key={index}
                        src={src}
                        alt={`Vista previa del auto ${index + 1}`}
