...
export default function CreateCarModal({
  ...
  // 🔹 Cargar categorías y preparar form
  useEffect(() => {
    if (!isOpen) {
  ...
  // 🔹 Ajustar categoría una vez cargadas
  useEffect(() => {
    if (categories.length > 0) {
      setFormData((prev) => {
        // si ya tiene categoría válida, no tocar
        if (prev.category_id && categories.some((c) => c.id === prev.category_id)) {
          return prev;
        }
        // si está creando uno nuevo, usar la primera categoría
