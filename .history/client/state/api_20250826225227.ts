...
export interface Car {
  ...
  pricePerHour: number;
  imagePaths: string[];
  characteristics?: string[]; // ⚠️ Agregado al tipo Car
  category?: {
    id: number;
    name: string;
  };
...
export interface NewCar {
  ...
  category_id: number;
  images?: File[];
  characteristics?: string[]; // ⚠️ Agregado a la interfaz NewCar
}
...
export const carApi = {
  ...
  create: async (newCar: NewCar): Promise<Car> => {
    ...
    formData.append(
      ...
      JSON.stringify({
        ...
        characteristics: newCar.characteristics, // ⚠️ Asegurándonos de que se envíe
      })
    );
  ...
  update: async (id: number, updatedCar: Partial<NewCar>): Promise<Car> => {
    const formData = new FormData();
    formData.append(
      "car",
      JSON.stringify({
        ...
        characteristics: updatedCar.characteristics, // ⚠️ Asegurándonos de que se envíe
      })
    );
