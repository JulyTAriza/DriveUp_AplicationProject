...
export interface Car {
  ...
  reservedDates?: string[]; // 👈 también lo agregamos en el tipo de respuesta
}
...
export interface NewCar {
  ...
  images?: File[];
  characteristics: string[];
  reservedDates?: string[]; // 👈 agregado para enviar
}
...
export const carApi = {
  ...
  create: async (newCar: NewCar): Promise<Car> => {
    ...
    formData.append(
      ...
      JSON.stringify({
        ...
        characteristics: newCar.characteristics,
        reservedDates: newCar.reservedDates || [], // 👈 agregado
      })
    );
  ...
  update: async (id: number, updatedCar: NewCar): Promise<Car> => {
    const formData = new FormData();
    formData.append(
      "car",
      JSON.stringify({
        ...
        category: { id: updatedCar.category_id },
        characteristics: updatedCar.characteristics,
        reservedDates: updatedCar.reservedDates || [], // 👈 agregado
      })
    );

    if (updatedCar.images && updatedCar.images.length > 0) {
      updatedCar.images.forEach((img) => {
        formData.append("carImages", img);
      });
    }
